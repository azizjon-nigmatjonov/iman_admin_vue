        ...
        <a-form-model-item>
          <a-row type="flex" justify="end">
            <a-col span="auto">
              ...
              <a-button type="primary" :loading="submitting" @click="submitIncome">{{ $t('button.save') }}</a-button>
            </a-col>
          </a-row>
        </a-form-model-item>
    ...
    submitIncome () {
      this.$refs.ruleForm.validate((valid, values) => {
        if (valid) {
          ...
          this.$store.dispatch('postIncome', this.form).then((res) => {
            // this.installment_list = this.installment_list.filter((el) => el.guid !== item.guid)
            this.$message.success('Income has added successfully')
            this.$router.go(-1)
          })
          .finally(() => {
