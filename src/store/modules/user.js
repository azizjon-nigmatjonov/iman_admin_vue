...
const user = {
  ...
  actions: {
    ...
    LoginUserSecond ({ commit }, userInfo2) {
      return new Promise((resolve, reject) => {
        login(userInfo2)
          .then(response => {
            storage.set(ACCESS_TOKEN_2, response.token, 7 * 24 * 60 * 60 * 1000)
            commit('SET_TOKEN_2', response.token)
            resolve()
          })
          .catch(error => {
            reject(error)
      ...
      return new Promise(resolve => {
        logout(state.token)
          ...
          .finally(() => {
            commit('SET_TOKEN_2', '')
            commit('SET_ROLES', [])
            storage.remove(ACCESS_TOKEN_2)
            storage.remove('user_id')
            storage.remove('roleList')
            storage.remove('client_id')
            storage.remove('refresh_token_2')
            storage.remove('user_role_id')
            location.reload()
          })
      })
